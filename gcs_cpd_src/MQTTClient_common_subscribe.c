...
#define CLIENTID1    "local_platform_common_sub"
#define CLIENTID2    "local_platform_common_pub"
...
int  gcs_sends_data_feedback(const char *reserved)
{
    ...
    int rc;
    char PAYLOAD[1024] = {0};                                                                      //array size: 1k        
...
{
	p_map = (char*)mmap(NULL, 1024, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);        //Shared memory size: 1K，type: char
	memset(p_map, 0, 1024);                                                                        //init p_map 0
    ...
    if (fpid < 0){   
		...
		MQTTCLIENT_PERSISTENCE_NONE, NULL);
		conn_opts.keepAliveInterval = 10;                                                          //set keepalive 10s
		conn_opts.cleansession = 1;
